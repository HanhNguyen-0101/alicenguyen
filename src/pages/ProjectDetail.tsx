...
import { MediaState, ProjectState } from "../redux/reducers/root.reducer";
import { useParams } from "react-router";
import { getProjectByIdRequest } from "../redux/actions/project.action";
import { history } from "../utils/history/history";
import { getMediaRequest } from "../redux/actions/media.action";
import { ProjectDetailComponent } from "../components/partials/ProjectDetailComponent";

export const ProjectDetail: React.FC<{}> = (props: any) => {
    ...
    useEffect(() => {
        ...
        dispatch(getProjectByIdRequest(callbackObj({ projectId: Number(id) }, callback("Inside callback after get project by id"))));
            ...
            <div className={`container-content overflow-y-auto ${customView.fontSize} ${customView.bright}`}>
                <div className="animated animatedFadeInUp fadeInUp relative mx-5 md:max-w-screen-sm lg:max-w-4xl md:mx-auto">
                    ...
                    <div className="max-w-xl lg:max-w-3xl px-6 py-6 md:py-10 lg:py-11 mx-auto space-y-6 md:space-y-10 lg:space-y-12">
                        {project && <ProjectDetailComponent mediaArr={mediaArr} project={project} customView={customView} />}
                        {!project && <article className="space-y-8">
